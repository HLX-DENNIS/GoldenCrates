...
public class RemovekeyCommand extends ISubCommand<GoldenCrates> {

	public RemovekeyCommand(@NotNull GoldenCrates plugin) {
		super(plugin, new String[] {"removekey"}, Perms.ADMIN);
	}

	@Override
	@NotNull
	public String usage() {
		return plugin.lang().Command_RemoveKey_Usage.getMsg();
	}

	@Override
	@NotNull
	public String description() {
		return plugin.lang().Command_RemoveKey_Desc.getMsg();
	}
	...
	public void perform(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
		...
		if (crateKey == null) {
			plugin.lang().Command_RemoveKey_Error_NoKey.send(sender);
			return;
		}
		...
		if (pName.equalsIgnoreCase(JStrings.MASK_ANY)) {
			for (Player player : plugin.getServer().getOnlinePlayers()) {
				...
				plugin.getKeyManager().takeKey(player.getName(), crateKey, amount);
			}
		}
		else {
		...
		plugin.lang().Command_RemoveKey_Done
